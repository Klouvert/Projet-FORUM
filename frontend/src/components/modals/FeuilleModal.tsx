...
const FeuilleModal = ({ noeud, onClose }: FeuilleModalProps) => {
  return (
        ...
        {noeud.amendements.length > 0 && (
          <div style={sectionStyle}>
            ...
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {noeud.amendements.map((amendement, i) => (
                <li key={amendement.id} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    ...
                    <span style={{ color: '#888', fontSize: '11px' }}>
                      {amendement.stats.average.toFixed(1)}/10 · {amendement.stats.totalVotes} votes
                    </span>
                  </div>
          ...
          <div style={sectionStyle}>
            ...
            <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '6px' }}>
              {noeud.arguments.map((arg) => (
                <li key={arg.id} style={{
                  padding: '8px 12px',
                  background: arg.type === 'pour' ? 'rgba(76,175,80,0.08)' : 'rgba(229,57,53,0.08)',
                  borderLeft: `2px solid ${arg.type === 'pour' ? '#4CAF50' : '#e53935'}`,
